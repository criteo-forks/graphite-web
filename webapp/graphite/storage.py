...
class Store(object):
    ...
    def fetch(self, patterns, startTime, endTime, now, requestContext):
        ...
        # Start fetches
        start = time.time()
        results = self.wait_jobs(jobs, settings.REMOTE_FETCH_TIMEOUT,
    ...
    def get_index(self, requestContext=None):
        ...
        start = time.time()
        results = self.wait_jobs(jobs, settings.REMOTE_FETCH_TIMEOUT, context)
    ...
    def tagdb_auto_complete_tags(self, exprs, tagPrefix=None, limit=None, requestContext=None):
        ...
        # Start fetches
        start = time.time()
        for result in self.wait_jobs(jobs, settings.REMOTE_FIND_TIMEOUT, context):
